...
class WaterConsumptionDao {
  ...
  Future<void> updateWaterConsumptionsDao(WaterConsumptionModel waterConsumptions) async {
    _db = await AppDatabase.instance.database;
    await _db.transaction((txn) async {
      await txn.update('waterConsumption', waterConsumptions.toMapDao(), where: "id = ?", whereArgs: [waterConsumptions.id]);
    });
  }
  ...
  Future<void> clearWaterConsumptionsDao() async {
    _db = await AppDatabase.instance.database;
    await _db.transaction((txn) async {
      await txn.delete('waterLog',);
      await txn.delete('waterConsumption',);
    });
  }
  ...
  Future<WaterConsumptionModel?> getNextSendOdooWaterConsumptionModel([String waterConsumptionIgnore = '']) async {
    if (waterConsumptionIgnore != '') waterConsumptionIgnore = ' and id > ($waterConsumptionIgnore)';

    _db = await AppDatabase.instance.database;
    var waterConsumptiosDao = await _db.query('waterConsumption', where: 'statesendserver = ${StateSendServerEnum.read_2}', limit: 1);

    if (waterConsumptiosDao.length == 0)
      waterConsumptiosDao = await _db.query('waterConsumption',
          where: 'statesendserver in (${StateSendServerEnum.sending_3}, ${StateSendServerEnum.sendingError_4}) $waterConsumptionIgnore',
          orderBy: 'id',
          limit: 1);

    if (waterConsumptiosDao.length == 0) return null;

    final WaterConsumptionModel waterConsumptionModel = WaterConsumptionModel.fromDao(waterConsumptiosDao[0]);
    return waterConsumptionModel;
  }

  Future<void> logout() async {
    _db = await AppDatabase.instance.database;
    // await _db.delete('waterConsumption'); //t. Pedro, creio que não seja necessário excluir waterConsumption quando fazer logout
  }

  Future<int?> waterLogInsert(int? waterConsumption_id, int? typelog, [int? loteprocess = 0, String? logText = null]) async {
    int? id;
    await _db.transaction((txn) async {
      id = await txn.insert(
          'waterLog', {'waterconsumption_id': waterConsumption_id, 'typelog': typelog, 'loteprocess': loteprocess, 'logtext': logText, 'datetime': DateTime.now().toString()});
      if (loteprocess == 0) await txn.update('waterLog', {'loteprocess': id}, where: "id = ?", whereArgs: [id]);
    });
    return id;
  }
  ...
  Future<dynamic?> getWaterConsumptionAmounts() async {
    ...
    String amountToSendSql = '${StateSendServerEnum.read_2}, ${StateSendServerEnum.sending_3}, ${StateSendServerEnum.sendingError_4}';
